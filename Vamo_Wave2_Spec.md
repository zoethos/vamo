...
| R5 | **Money governance I**: expense `status` (proposed/committed/cancelled), share `response` (pending/accepted/rejected+reason); settle counts committed only; **hard display rule** (disputed ≠ silent) | state machine per memo incl. A1-netting dispute window; share invariant tests; balances show "disputed by X" |
