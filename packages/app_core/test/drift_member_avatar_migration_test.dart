...
void main() {
  test('schema v19 includes local trip member avatar_url column', () async {
  ...
  test('v18 to v19 migration adds local trip member avatar_url column', () async {
...
''');
    ...
    await db.upsertMember(
      LocalTripMembersCompanion(
        ...
        status: const Value('active'),
        displayName: const Value('Sam'),
        avatarUrl: const Value('user-2/profile.jpg'),
        completedAt: Value(now),
      ),
    );
    ...
    final rows = await db.watchActiveMembers('trip-1').first;
    expect(rows, hasLength(1));
    expect(rows.first.avatarUrl, 'user-2/profile.jpg');
  });
}
