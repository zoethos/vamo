-- S50 trust-boundary fixups (round 2):
-- 1) BEFORE DELETE trigger must return OLD (NEW is NULL → delete skipped).
-- 2) insert_committed_expense recomputes equal split; client ids preserved by user_id.
...
begin
  ...
  -- BEFORE DELETE must return OLD to let the delete proceed
  return old;
end;
$$;
