...
|---|---|---|---|---|
| 1 | **No back affordance on mobile detail screen.** Trip/event detail has a back arrow on web (go_router/browser) but **none on Android** — can't return to the trips/home view from the screen. | trip detail and/or event detail; confirm which | Likely pre-existing (S17.1-era app-bar rework). If it's the S21 **event detail** specifically, it's an S21 regression → fix before merge. | High (navigation dead-end) |
| 2 | **Add-expense action shows on ALL trip tabs** (Expense, Plan, Balance, Members), not just Expenses — fixed FAB ignoring context. Should be per-tab: Expense → add expense; Plan → add plan item; Members → invite member; Balance → no add (read-only). | trip tab scaffold / FAB | Pre-existing FAB behavior (not S21). | Medium (wrong action on 3 tabs) |
...
## Suggested fix order for Cursor (one batch)
...
2. **P1 — S21 create-event usable (blocks S21 merge):** #4 relabel type dropdown
...
3. **P1 — navigation:** #1 add back affordance on mobile detail screens.
4. **P2 — polish:** #2 context-aware "+" (Plan vs Expenses); #3/#3a lime-on-light
   audit + token annotation; #5 event-type badge inside the event.

After this batch lands → one clean device pass = verifies the fixes **and** the
S21 two-user realtime check in a single run.
...
## Notes
- Confirm finding #1's screen: if the missing back button is on the **event
  detail** (opened from Plan, S21), fold the fix into the S21 PR; if it's the
  **trip detail**, it's a separate nav fix.
- Both are mobile-vs-web parity issues — web has correct chrome, Android is
  missing it. Check the Scaffold/AppBar `leading` / `automaticallyImplyLeading`
  on the mobile route, and the FAB's per-tab visibility logic.
