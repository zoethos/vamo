...
## 3. What S17/S22 will add

| Job | Trigger | Notes |
|-----|---------|-------|
| Trip auto-unresolved | daily | `end_date + 6 months` (warn at 5) |
| Settle-up nudge | daily after `end_date` | needs push (R2) + open balances |

Both call the same heartbeat table for observability and reuse whichever
scheduler is live.
