...
`K6_ALLOW_NON_STAGING=true` is explicitly set for another non-production
target. It always refuses the known production project ref.
...
- Do not run the k6 script against production. Use staging or another
  intentionally isolated non-production project.
