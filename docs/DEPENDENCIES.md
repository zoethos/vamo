...
| **OpenAI** (S23 theme miss path) | T2 | New destinations fall back to the default/local theme pack | cache-first design; existing `trips.theme` values and built-in fallback still render | Provision `OPENAI_API_KEY` before S23; log throttles/timeouts and never block trip creation. |
...
| **OpenAI** | usage-based small-model calls | usage-based | distinct uncached destinations only | cache-miss count + `provider_throttled` |
...
| **OpenAI** | S23 AI theme generation on cache miss | `OPENAI_API_KEY` as Supabase Edge Function secret; never client-side | usage-based; expected negligible due global cache | Low-med — schema prompt + validation are ours | model/pricing changes; sustained 429/5xx; output quality review |
...
| `OPENAI_API_KEY` | Supabase Edge Function secret | `resolve-theme` (S23); server-only |
