...
const consoleRuntimeWithFsqCredential = consoleRuntimeFiles.filter((file) => {
  const source = readFileSync(file, "utf8");
  return /FSQ_OS_PLACES_CATALOG_(?:SERVICE_API_KEY|TOKEN)/.test(source);
});

assert(
  consoleRuntimeWithFsqCredential.length === 0,
  `console runtime must not reference FSQ OS Places credentials:\n${consoleRuntimeWithFsqCredential
    .map(toRepoRelative)
    .join("\n")}`
);
